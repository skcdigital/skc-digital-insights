import { createFileRoute } from "@tanstack/react-router";
import { supabaseAdmin } from "@/integrations/supabase/client.server";

// PayFast hosted payment page URLs
const PF_LIVE_URL = "https://www.payfast.co.za/eng/process";
const PF_SANDBOX_URL = "https://sandbox.payfast.co.za/eng/process";

export const Route = createFileRoute("/api/payfast/checkout")({
  server: {
    handlers: {
      POST: async ({ request }) => {
        const PAYFAST_MERCHANT_ID = process.env.PAYFAST_MERCHANT_ID;
        const PAYFAST_MERCHANT_KEY = process.env.PAYFAST_MERCHANT_KEY;
        const PAYFAST_PASSPHRASE = process.env.PAYFAST_PASSPHRASE ?? "";
        const PAYFAST_SANDBOX = process.env.PAYFAST_SANDBOX === "true";
        const SITE_URL = process.env.SITE_URL ?? "https://www.skcdigital.co.za";

        if (!PAYFAST_MERCHANT_ID || !PAYFAST_MERCHANT_KEY) {
          return Response.json({ error: "Payment gateway not configured." }, { status: 500 });
        }

        let body: Record<string, unknown>;
        try {
          body = await request.json();
        } catch {
          return Response.json({ error: "Invalid JSON." }, { status: 400 });
        }

        const str = (v: unknown, max = 500) =>
          typeof v === "string" ? v.trim().slice(0, max) : "";

        const type = str(body.type, 20) as "product" | "membership";
        const itemId = str(body.itemId, 100);
        const email = str(body.email, 255);
        const billing = str(body.billing, 20) as "monthly" | "annual";

        if (!type || !itemId || !email) {
          return Response.json({ error: "Missing required fields." }, { status: 400 });
        }

        let amountZar: number;
        let itemName: string;
        let internalPaymentId: string;

        if (type === "product") {
          const { data: product, error } = await supabaseAdmin
            .from("products")
            .select("id, title, price_zar, is_published, is_free")
            .eq("id", itemId)
            .maybeSingle();

          if (error || !product) {
            return Response.json({ error: "Product not found." }, { status: 404 });
          }
          const p = product as { id: string; title: string; price_zar: number; is_published: boolean; is_free: boolean };
          if (!p.is_published) return Response.json({ error: "Product not available." }, { status: 403 });
          if (p.is_free) return Response.json({ error: "This product is free." }, { status: 400 });

          amountZar = p.price_zar;
          itemName = p.title.slice(0, 100);

          // Pre-create a pending purchase record
          const { data: purchase } = await supabaseAdmin
            .from("purchases")
            .insert({ product_id: p.id, email, amount_zar: amountZar, status: "pending" })
            .select("id")
            .single();

          internalPaymentId = `prod-${(purchase as { id: string } | null)?.id ?? p.id}`;

        } else if (type === "membership") {
          const { data: plan, error } = await supabaseAdmin
            .from("membership_plans")
            .select("id, name, price_monthly, price_annual, is_active")
            .eq("id", itemId)
            .maybeSingle();

          if (error || !plan) return Response.json({ error: "Plan not found." }, { status: 404 });
          const pl = plan as { id: string; name: string; price_monthly: number; price_annual: number | null; is_active: boolean };
          if (!pl.is_active) return Response.json({ error: "Plan not available." }, { status: 403 });

          amountZar = billing === "annual" && pl.price_annual != null
            ? pl.price_annual
            : pl.price_monthly;
          itemName = `SKC Digital — ${pl.name} (${billing === "annual" ? "Annual" : "1st Month"})`.slice(0, 100);
          internalPaymentId = `mem-${pl.id}-${billing}-${Date.now()}`;

        } else {
          return Response.json({ error: "Invalid type." }, { status: 400 });
        }

        // Build PayFast fields — ORDER MATTERS for signature
        const fields: Record<string, string> = {
          merchant_id: PAYFAST_MERCHANT_ID,
          merchant_key: PAYFAST_MERCHANT_KEY,
          return_url: `${SITE_URL}/checkout/success`,
          cancel_url: `${SITE_URL}/${type === "product" ? "products" : "memberships"}`,
          notify_url: `${SITE_URL}/api/payfast/notify`,
          email_address: email,
          m_payment_id: internalPaymentId,
          amount: amountZar.toFixed(2),
          item_name: itemName,
        };

        const signature = await generateSignature(fields, PAYFAST_PASSPHRASE);
        const paymentUrl = PAYFAST_SANDBOX ? PF_SANDBOX_URL : PF_LIVE_URL;

        return Response.json({ paymentUrl, fields: { ...fields, signature } });
      },
    },
  },
});

// ---------------------------------------------------------------------------
// PayFast signature: MD5 of URL-encoded key=value pairs + passphrase
// ---------------------------------------------------------------------------
async function generateSignature(
  fields: Record<string, string>,
  passphrase: string
): Promise<string> {
  let payload = Object.entries(fields)
    .map(([k, v]) => `${k}=${encodeURIComponent(v.trim()).replace(/%20/g, "+")}`)
    .join("&");

  if (passphrase) {
    payload += `&passphrase=${encodeURIComponent(passphrase.trim()).replace(/%20/g, "+")}`;
  }

  // MD5 via SubtleCrypto isn't available — use a pure-JS implementation
  return md5(payload);
}

// Pure-JS MD5 (no native crypto dependency — works in Cloudflare Workers)
function md5(input: string): string {
  const bytes = new TextEncoder().encode(input);
  const hex = (n: number) => n.toString(16).padStart(2, "0");

  function safeAdd(x: number, y: number) {
    const lsw = (x & 0xffff) + (y & 0xffff);
    const msw = (x >> 16) + (y >> 16) + (lsw >> 16);
    return (msw << 16) | (lsw & 0xffff);
  }
  function bitRotateLeft(num: number, cnt: number) {
    return (num << cnt) | (num >>> (32 - cnt));
  }
  function md5cmn(q: number, a: number, b: number, x: number, s: number, t: number) {
    return safeAdd(bitRotateLeft(safeAdd(safeAdd(a, q), safeAdd(x, t)), s), b);
  }
  function md5ff(a: number, b: number, c: number, d: number, x: number, s: number, t: number) {
    return md5cmn((b & c) | (~b & d), a, b, x, s, t);
  }
  function md5gg(a: number, b: number, c: number, d: number, x: number, s: number, t: number) {
    return md5cmn((b & d) | (c & ~d), a, b, x, s, t);
  }
  function md5hh(a: number, b: number, c: number, d: number, x: number, s: number, t: number) {
    return md5cmn(b ^ c ^ d, a, b, x, s, t);
  }
  function md5ii(a: number, b: number, c: number, d: number, x: number, s: number, t: number) {
    return md5cmn(c ^ (b | ~d), a, b, x, s, t);
  }

  // Prepare message
  const len8 = bytes.length;
  const len32 = Math.ceil((len8 + 9) / 64) * 16;
  const M = new Int32Array(len32);
  for (let i = 0; i < len8; i++) M[i >> 2] |= bytes[i] << ((i % 4) * 8);
  M[len8 >> 2] |= 0x80 << ((len8 % 4) * 8);
  M[len32 - 2] = len8 * 8;

  let a = 1732584193, b = -271733879, c = -1732584194, d = 271733878;

  for (let i = 0; i < len32; i += 16) {
    const [oA, oB, oC, oD] = [a, b, c, d];
    a = md5ff(a,b,c,d,M[i+ 0], 7,-680876936); d=md5ff(d,a,b,c,M[i+ 1],12,-389564586); c=md5ff(c,d,a,b,M[i+ 2],17, 606105819); b=md5ff(b,c,d,a,M[i+ 3],22,-1044525330);
    a = md5ff(a,b,c,d,M[i+ 4], 7,-176418897); d=md5ff(d,a,b,c,M[i+ 5],12, 1200080426); c=md5ff(c,d,a,b,M[i+ 6],17,-1473231341); b=md5ff(b,c,d,a,M[i+ 7],22,-45705983);
    a = md5ff(a,b,c,d,M[i+ 8], 7, 1770035416); d=md5ff(d,a,b,c,M[i+ 9],12,-1958414417); c=md5ff(c,d,a,b,M[i+10],17,-42063); b=md5ff(b,c,d,a,M[i+11],22,-1990404162);
    a = md5ff(a,b,c,d,M[i+12], 7, 1804603682); d=md5ff(d,a,b,c,M[i+13],12,-40341101); c=md5ff(c,d,a,b,M[i+14],17,-1502002290); b=md5ff(b,c,d,a,M[i+15],22, 1236535329);
    a = md5gg(a,b,c,d,M[i+ 1], 5,-165796510); d=md5gg(d,a,b,c,M[i+ 6], 9,-1069501632); c=md5gg(c,d,a,b,M[i+11],14, 643717713); b=md5gg(b,c,d,a,M[i+ 0],20,-373897302);
    a = md5gg(a,b,c,d,M[i+ 5], 5,-701558691); d=md5gg(d,a,b,c,M[i+10], 9, 38016083); c=md5gg(c,d,a,b,M[i+15],14,-660478335); b=md5gg(b,c,d,a,M[i+ 4],20,-405537848);
    a = md5gg(a,b,c,d,M[i+ 9], 5, 568446438); d=md5gg(d,a,b,c,M[i+14], 9,-1019803690); c=md5gg(c,d,a,b,M[i+ 3],14,-187363961); b=md5gg(b,c,d,a,M[i+ 8],20, 1163531501);
    a = md5gg(a,b,c,d,M[i+13], 5,-1444681467); d=md5gg(d,a,b,c,M[i+ 2], 9,-51403784); c=md5gg(c,d,a,b,M[i+ 7],14, 1735328473); b=md5gg(b,c,d,a,M[i+12],20,-1926607734);
    a = md5hh(a,b,c,d,M[i+ 5], 4,-378558); d=md5hh(d,a,b,c,M[i+ 8],11,-2022574463); c=md5hh(c,d,a,b,M[i+11],16, 1839030562); b=md5hh(b,c,d,a,M[i+14],23,-35309556);
    a = md5hh(a,b,c,d,M[i+ 1], 4,-1530992060); d=md5hh(d,a,b,c,M[i+ 4],11, 1272893353); c=md5hh(c,d,a,b,M[i+ 7],16,-155497632); b=md5hh(b,c,d,a,M[i+10],23,-1094730640);
    a = md5hh(a,b,c,d,M[i+13], 4, 681279174); d=md5hh(d,a,b,c,M[i+ 0],11,-358537222); c=md5hh(c,d,a,b,M[i+ 3],16,-722521979); b=md5hh(b,c,d,a,M[i+ 6],23, 76029189);
    a = md5hh(a,b,c,d,M[i+ 9], 4,-640364487); d=md5hh(d,a,b,c,M[i+12],11,-421815835); c=md5hh(c,d,a,b,M[i+15],16, 530742520); b=md5hh(b,c,d,a,M[i+ 2],23,-995338651);
    a = md5ii(a,b,c,d,M[i+ 0], 6,-198630844); d=md5ii(d,a,b,c,M[i+ 7],10, 1126891415); c=md5ii(c,d,a,b,M[i+14],15,-1416354905); b=md5ii(b,c,d,a,M[i+ 5],21,-57434055);
    a = md5ii(a,b,c,d,M[i+12], 6, 1700485571); d=md5ii(d,a,b,c,M[i+ 3],10,-1894986606); c=md5ii(c,d,a,b,M[i+10],15,-1051523); b=md5ii(b,c,d,a,M[i+ 1],21,-2054922799);
    a = md5ii(a,b,c,d,M[i+ 8], 6, 1873313359); d=md5ii(d,a,b,c,M[i+15],10,-30611744); c=md5ii(c,d,a,b,M[i+ 6],15,-1560198380); b=md5ii(b,c,d,a,M[i+13],21, 1309151649);
    a = md5ii(a,b,c,d,M[i+ 4], 6,-145523070); d=md5ii(d,a,b,c,M[i+11],10,-1120210379); c=md5ii(c,d,a,b,M[i+ 2],15, 718787259); b=md5ii(b,c,d,a,M[i+ 9],21,-343485551);
    a = safeAdd(a, oA); b = safeAdd(b, oB); c = safeAdd(c, oC); d = safeAdd(d, oD);
  }

  return [a,b,c,d].map(n => {
    let s = "";
    for (let i = 0; i < 4; i++) s += hex((n >> (i * 8)) & 0xff);
    return s;
  }).join("");
}
